...
// [Bruno Levy] define an alternative version
// of ANNpointArray, that avoids needing to
// store pointers if data is contiguous in
// memory.
#define ANN_CONTIGUOUS_POINT_ARRAY 
#ifdef ANN_CONTIGUOUS_POINT_ARRAY
class ANNpointArray {
 public:
   ANNpointArray(
       const ANNcoord* data, size_t stride
   ) : data_(const_cast<ANNcoord*>(data)), stride_(stride) {
   }
   ANNpointArray() : data_(NULL), stride_(0) {
   }

   // This constructor is needed for allowing NULL
   // value (or default argument) for ANNpointArray.
#ifdef NDEBUG
   ANNpointArray(void*) : data_(NULL), stride_(0) {
   }
#else
   ANNpointArray(void* p) : data_(NULL), stride_(0) {
	if(p != NULL) { abort() ; }
   }
#endif

   ANNpoint operator[](size_t i) {
