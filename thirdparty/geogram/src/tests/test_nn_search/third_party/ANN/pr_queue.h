...
public:
	...
	// [Bruno Levy] 
	ANNpr_queue(int max, void* pq_in)	// constructor (given max size)
		{
			n = 0;			// initially empty
			max_size = max;		// maximum number of items
			pq = (pq_node*)pq_in;	// queue is array [1..max] of nodes
			owns_pq = false ;
		}

	~ANNpr_queue()						// destructor
	{ if(owns_pq) { delete [] pq; } }
