...
//		consecutively.)
//----------------------------------------------------------------------
const ANNcoord BD_GAP_THRESH = 0.5;		// gap threshold (must be < 1)
	...
	ANNorthRect			&inner_box)		// inner box if shrinking (returned)
{
	...
	int n_goal = (int) ((float)n*BD_FRACTION); // number of point in goal
	...
	while (n_sub > n_goal) {			// keep splitting until goal reached
        ...
        if ((float)n_splits > (float)dim*BD_MAX_SPLIT_FAC)// took too many splits
