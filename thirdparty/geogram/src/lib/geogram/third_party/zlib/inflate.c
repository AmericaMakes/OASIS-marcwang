...
/* [Bruno] full path to geogram to avoid including wrong zlib header */    
#   include <geogram/third_party/zlib/inffixed.h>
...
/* [Bruno] added (unsigned long) / (long) casts to avoid a warning */
    if (strm == Z_NULL || strm->state == Z_NULL) return (long)((unsigned long)(-1L) << 16);
    state = (struct inflate_state FAR *)strm->state;
